...
class List extends Component {
    ...
    ajaxReq.call(this,{
      ...
      fixed.push({
        ...
        render: (row) => {
          return (
            <span>
              <Button onClick={this.showInfo.bind(this, row)} plain={true} type="info" size="small">详情</Button>
              <Button onClick={this.update.bind(this, row)} type="primary" size="small">编辑</Button>
              <Button onClick={this.delInfo.bind(this, row)} type="danger" size="small">删除</Button>
            </span>
          )
        }
      })
  ...
  render() {
    return (
      <div className="list">
        <Loading loading={this.state.loading} text="拼命加载中..." style={{width: '100%', height: '100%'}}>
          <div className="list-table">
            ...
            {/* page */
              ...
              <div className="list-page">
                <Pagination onCurrentChange={this.changePage} layout="prev, pager, next" total={this.state.total}/>
              </div>
            ) : null}
          </div>
        </Loading>
        {this.state.isShow ? <Box show={()=>this.setState({isShow: false})} data={this.state.info}/> : null}
      </div>
    )
  }
}
