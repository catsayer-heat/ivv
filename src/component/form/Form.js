...
class Form extends Component {
    ...
    setForm = (data, isShow) => {
        return data.map((val, index)=>{
            return (
                <li key={index} className={this.state.save&&this.state.index===index&&this.state.isShow===isShow?'form-cnt line':'form-cnt'}>
                    <span>{index+1}</span>
                    <span>{val.name}</span>
    ...
    handleUp = (index, isShow) => {
        ...
        if(isShow) {
            ...
            ;[showForm[index], showForm[index-1]] = [showForm[index-1], showForm[index]]
            this.setState({showForm})
        } else {
            const fixForm = this.state.fixForm
            ;[fixForm[index], fixForm[index-1]] = [fixForm[index-1], fixForm[index]]
    ...
    setShowSet = (data, isShow) => {
        ...
        if(this.state.isShowSet) {
            if(isShow) { //判断是否是确认
                if(data.sign==="custom") {
                    const showForm = this.state.showForm
                    this.state.index===-1 ? showForm.push(data) : (showForm[this.state.index]=data)
                    this.setState({showForm})
                } else {
                    const fixForm = this.state.fixForm
                    this.state.index===-1 ? fixForm.push(data) : (fixForm[this.state.index]=data)
                    this.setState({fixForm})
                }
        ...
        } else { //false 时接收的data 表示的是index数据   （操作的父组件）
            // 编辑数组时设置传递的数据
            const x = data===-1?{}:isShow ? JSON.parse(JSON.stringify(this.state.showForm[data])) : JSON.parse(JSON.stringify(this.state.fixForm[data]))
            this.setState({
                index: data,
                data: x,
    ...
    render() {
        return (
            <div className="form">
                <Loading loading={this.state.loading} style={{width: '100%', height: '100%', minWidth: '900px'}} text="拼命加载中">
                    <div className="form-layout">
                        ...
                        <ul>
                            {this.setForm(this.state.showForm, true)}
                        </ul>
                        <div className="form-name">
                            展示字段
                        </div>
                        <ul>
                            {this.setForm(this.state.fixForm, false)}
                        </ul>
                    </div>
                    <span onClick={this.setShowSet.bind(this, -1)} className="add-form">
                        <i className="el-icon-plus"></i>
                    </span>
                    {this.state.save ? (<div className="form-btn">
                        <Button onClick={this.handleCancel}>取消</Button>
                        <Button onClick={this.handleSave}>保存</Button>
                    </div>) : null}
                </Loading>
                {this.state.isShowSet ? <Set data={this.state.data} show={this.setShowSet}/> : null}
            </div>
        )
    }
}
